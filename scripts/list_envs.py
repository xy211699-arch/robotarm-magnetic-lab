...
All the environments are registered in the `robotarm_magnetic_lab` extension. They start
...
# Keep this helper compatible with ``run_isaaclab.sh``, which injects Kit
# renderer workarounds through ``--kit_args`` for every Python entry point.
...
with contextlib.suppress(ImportError):
    import robotarm_magnetic_lab.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~robotarm_magnetic_lab.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from robotarm_magnetic_lab.utils.preset_target import PresetTarget
