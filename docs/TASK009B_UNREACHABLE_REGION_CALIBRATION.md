...
若配置已经存在，工具默认恢复全部既有种子，可继续追加、删除或调整半径，再保存为新版本。
因此可以每次只标记一个区域并退出，下次继续。若需要从空白草稿重新开始，增加 `--new`；
原配置在按下保存键之前保持不变。
...
- `B`：第一次记录包围盒起点；移动游标后再次按 `B`，完成世界坐标轴对齐包围盒。
- `X`：删除最后一个已完成的包围盒。
- `Tab`：切换当前种子。
- `[` / `]`：将当前种子的测地半径在 10–80 mm 间以 5 mm 切换。
- `Backspace`：删除当前种子；`C`：清除全部种子。
- `Enter`：冻结、保存并重新加载校验；`Esc`：退出且不保存。

红色网格是多个种子测地区域与多个包围盒的并集，绿色点为种子，黄色点为三维游标和
包围盒边线。包围盒采用世界坐标AABB，选择规则为“三角面质心位于包围盒内”；边界包含在内，
任一轴不足1 mm时自动扩展为1 mm厚度。保存前必须检查红色预览，避免同时选中胃腔对侧胃壁。
...
其中包括种子世界坐标、面片编号、各自半径、包围盒世界坐标边界、各选择区域面片、并集面片、
剩余可达面片、面积、
固定审计理由、操作者、胃模型哈希、面积权重哈希和配置哈希。固定理由为“操作者确认的物理/
解剖不可达区域”，不再要求通过命令行填写。
...
胃模型几何哈希、种子、半径或包围盒任一变化，都会产生新的配置哈希。正式数据集、训练运行
