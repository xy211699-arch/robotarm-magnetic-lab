...
`capsule_camera.cfg.offset.pos`，将离相机光心更近的半球中心认定为相机端；不再仅凭固定
局部轴符号猜测施力端。
...
- UP：实际相机端半球球心为 `p_cam`，在该点施加 `r_up*m*g*z_w`；非相机端不施加主动
  力。该动作保留重力和接触约束，由偏心点力自然产生抬升力矩。

MOVE/VIEW 的多个端点力通过等价质心力/力矩一次性提交到 PhysX：
...
UP 不走上述等价显式力矩路径：控制器向 PhysX 提交世界坐标力和世界坐标作用点
`positions=p_cam`，同时显式力矩为 `None`。因此旋转效应只来源于单点力的真实作用位置，
不是额外的控制力矩或力偶。
...
`(1.078678615084, 0.177197008592, 0.011793035807) m`，保持既有侧躺姿态。X/Y由实际下壁
网格采样；Z经过完整胶囊轴线201点净空扫描，初始最小净空约0.573 mm，随后由重力自然
落稳；TASK-003基线复位点不变。
