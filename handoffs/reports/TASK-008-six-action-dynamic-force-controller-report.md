...
按用户后续要求新增 `scripts/dynamic_force_macro/teleop_table.py`。该入口不读取、不伪装冻结
profile，而是显式显示本轮诊断候选 `MOVE=0.9mg, VIEW=3.0mg, UP=2.197265625mg`；提供
外部视口、胶囊相机和状态窗口，一次按键只执行一个1秒宏动作，并输出同标定器一致的指标。

六动作无窗口冒烟正常退出并保存六张边界RGB：HOLD、双向MOVE、双向VIEW在该连续顺序下
通过，UP的边界指标为 `camera_elevation_deg=-64.8058`、`crossed_vertical=false`。该值受
前五个动作累积状态影响，不能单独证明UP方向错误；用户可视验收确认UP方向正确、问题为
力度过大。该单次诊断不改变20样本标定结论，也不是胃部验收。
