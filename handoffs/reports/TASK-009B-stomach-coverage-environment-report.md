...
更新后的自动化结果见本报告对应提交；现场动力学定位、按键确认和区域画面仍必须由本机图形
会话验收。
...
按键及验收方法见`docs/TASK009B_ENTRY_ANCHOR_REGION_CALIBRATION.md`。用户应先用
MOVE/VIEW/UP选择位置和姿态并确认稳定锚点，再切换测地半径并确认单连通入口区域。将终端中的
