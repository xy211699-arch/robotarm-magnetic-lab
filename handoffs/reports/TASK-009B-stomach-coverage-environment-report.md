...
- 通过胃壁三角形共享边邻接图和Dijkstra距离生成10/15/20/25/30/60 mm测地区域；
...
首次现场WASD记录显示候选位置产生水平偏移且`simulation_time_s=0.0`，验证暂停移动主链路；
Kit键盘对部分按键返回裸字符串而非Input对象导致一次回调异常，现已兼容两种事件格式。

用户已完成一次30 mm入口区域保存：种子面片17589、1375个三角面、752个顶点、面积
0.0017662011374897796 m2、单连通分量，锚点哈希为
`9b5b33fef14bd183133818b60beac2d5b660d49a330f4a208023c3c9fd57bcec`。随后要求将半径扩大
到60 mm。工具现支持`--resume_anchor --initial_radius_mm 60`：校验锚点与当前胃壁哈希后，
直接恢复稳定锚点并重新计算区域，不重复自然下落、不手工篡改派生JSON。60 mm区域仍需现场
观察并按Enter确认，因此Gate 2在新区域保存前继续为`needs_input`。
